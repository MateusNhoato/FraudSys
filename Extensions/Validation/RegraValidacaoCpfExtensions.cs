...
namespace FraudSys.Extensions.Validation
{
    public static class RegraValidacaoCpfExtensions
    {
       ...
       this IRuleBuilder<T, string?> ruleBuilder, string? nomeCampo)
        {
            return ruleBuilder.NotEmpty().WithMessage(FraudSysResource.CpfEhObrigatorio)
                                         .Matches(@"^\d{3}\.?\d{3}\.?\d{3}-?\d{2}$").WithMessage( string.Format(FraudSysResource.FormatoCpfInvalido, (string.IsNullOrEmpty(nomeCampo) ? string.Empty : nomeCampo + " ")));
        }
    }
}
